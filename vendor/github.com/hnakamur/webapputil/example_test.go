...
func ExampleWithErrorHandler() {
	...
	// NOTE: Functions here are defined as variables due to the limitation of showing examples in godoc
	// since example codes must be wrapped with ExampleXXX functions and nested functions are not
	// allowed in Go.
	...
	//	}
	//
	// In real code, you can define functions like:
	//	func errorHandler(hErr *webapputil.HTTPError, w http.ResponseWriter, r *http.Request) {
	//		...
	//	}

	errorHandler := func(hErr *webapputil.HTTPError, w http.ResponseWriter, r *http.Request) {
	...
	sendResponse := func(w http.ResponseWriter, statusCode int, detail interface{}) error {
	...
	handleRoot := func(w http.ResponseWriter, r *http.Request) *webapputil.HTTPError {
		err := r.ParseForm()
		if err != nil {
